import { IBlog, ICategory, IComment, type PagingDefault, type PagingSearch } from '@/interface'
...
export const getLatestBlogs = (limit = 40, page = 1): Promise<PagingSearch<IBlog[]>> => {
...
// 根据分类id 获取文章
export const getBlogsByCategoryId = (cateid: string, limit = 40, page = 1): Promise<PagingSearch<IBlog[], { cateName: string }>> => {
...
// 根据年份获取文章
export const getBlogsByYear = (year: string, limit = 4, page = 1): Promise<PagingSearch<IBlog[], { year: number, start: number, end: number }>> => {
...
export const getBlogComments = (logid: string, limit = 5, page = 1): Promise<PagingDefault<IComment[]>> => {
  return request('GET_COMMENTS', { params: { logid, limit, page }, defaultResult: { data: [], total: 0 } })
}
